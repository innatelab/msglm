functions {
    ...
    // count nonzero elements in obsXobs0 matrix
    // (block-diagonal matrix with block generated by contr_poly())
    int count_obsXobs0_Nw(int niactions, int[] observation2iaction) {
        int nobs[niactions];
        int nw;
        for (i in 1:niactions) nobs[i] = 0;
        for (i in 1:num_elements(observation2iaction)) nobs[observation2iaction[i]] += 1;
        nw = 0;
        for (i in 1:niactions) nw += (nobs[i]-1)*nobs[i];
        return nw;
    }
}
...
transformed data {
  ...
  vector[count_obsXobs0_Nw(Niactions, observation2iaction)] obsXobs_shift0_w;
  int<lower=1, upper=count_obsXobs0_Nw(Niactions, observation2iaction) + 1> obsXobs_shift0_u[Nobservations + 1];
  int<lower=1, upper=Nobservations - Niactions> obsXobs_shift0_v[count_obsXobs0_Nw(Niactions, observation2iaction)];
  ...
  obsXobs0_Nw = count_obsXobs0_Nw(Niactions, observation2iaction);
  {
