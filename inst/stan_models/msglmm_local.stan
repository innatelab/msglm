...
transformed data {
  ...
  int<lower=1,upper=Nmsprotocols*Nobjects> quant2msprotoXobj[((Nmsprotocols > 1) && (Nobjects > 0)) ? Nquanted : 0];
  int<lower=1,upper=Nsupactions> miss2supaction[Nmissed];
  int<lower=1,upper=Nexperiments> miss2experiment[Nmissed];
  int<lower=1,upper=Nmsprotocols*Nobjects> miss2msprotoXobj[((Nmsprotocols > 1) && (Nobjects > 0)) ? Nmissed : 0];
  ...
  if (Nobjects > 0) {
    if (Nmsprotocols > 1) {
      ...
      for (i in 1:Nquanted) {
        ...
        quant2msprotoXobj[i] = msproto > 1 ? (msproto-1)*Nobjects + supaction2obj[observation2supaction[quant2observation[i]]] + 1 : 1;
      }
      for (i in 1:Nmissed) {
        int msproto;
        msproto = experiment2msproto[quant2experiment[i]];
        miss2msprotoXobj[i] = msproto > 1 ? (msproto-1)*Nobjects + supaction2obj[observation2supaction[miss2observation[i]]] + 1 : 1;
      }
    }
  }
}
...
parameters {
  ...
  real<lower=0.0> msproto_shift_sigma;
  vector[Nobjects > 0 && Nmsprotocols > 1 ? Nobjects * (Nmsprotocols-1) : 0] obj_msproto_shift_unscaled;
...
model {
    ...
    msproto_shift_sigma ~ inv_gamma(2.0, 1.0); // if Nmsprotocols==1, fake something far from 0 and +Inf
    if (Nmsprotocols > 1) {
      to_vector(obj_msproto_shift_unscaled) ~ normal(0.0, 1.0);
    }
    ...
    // calculate the likelihood
    {
        ...
        if (Nmsprotocols > 1) {
            vector[(Nmsprotocols-1)*Nobjects+1] msproto_shift;
            msproto_shift[1] = 0.0; // the 1st protocol has no shift (taken care by suo_shift)
            msproto_shift[2:((Nmsprotocols-1)*Nobjects+1)] = obj_msproto_shift_unscaled * msproto_shift_sigma;
            q_labu += msproto_shift[quant2msprotoXobj];
            m_labu += msproto_shift[miss2msprotoXobj];
        }
        if (NbatchEffects > 0) {
