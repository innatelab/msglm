...
test_that("msglm_model()", {
  ...
  expect_error(empty_model <- msglm_model(constant_matrix(1, list(condition = c(), effect = c())),
                            conditions = data.frame(condition = character()),
                            effects = data.frame(effect = character())), "Assertion on 'conditionXeffect' failed: Must have at least 1 rows, but has 0 rows.")
})
